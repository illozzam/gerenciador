    ...
    if not isPivot:
        ...
        for i in range(1, len(points)):
            ...
            if (len(connections[pointStr]) > 2) or (pointStr in pivotPoints):
                ...
                lineKey = (
                    format % line[-1] + ":" + format % line[-2] + ":" + format % line[0]
                )
                if lineKey in simplifiedLines:
                    ...
                    simpleLine = (
                        shapely.geometry.LineString(line).simplify(tolerance).coords
                    )
                    lineKey = (
                        format % line[0]
                        + ":"
...
while in_feat is not None:
    if results[counter] is not None:
        ...
        out_feat.SetGeometryDirectly(
            ogr.CreateGeometryFromWkb(shapely.wkb.dumps(results[counter]))
        )
